...
from PySide6.QtGui import QDesktopServices, QTextDocument, QTextCursor
...
# МНОГОПОТОЧНОСТЬ
    ...
    def algorith(self, data, range_max_min, start_ratio=[0.2, 0.1, 0.1]):
        ...
        # data_graph = pd.concat([data, rr], axis=1)

        # data_graph['assessment'] = 0
        return rr
    ...
    def plot(self, widget_name, data_x, data_y, label, label_x, label_y):
        ...
        if pd.api.types.is_datetime64_any_dtype(data_x):
            ...
            widget_name.figure.autofmt_xdate()  # Автоматический поворот дат
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    #     self.display_table(self.ui.table_classification, data)
